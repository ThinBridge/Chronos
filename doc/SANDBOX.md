...
## サブプロセスを別EXEにできるか(結論: サンドボックスとは両立しない)

「レンダラー等のサブプロセスをメインの `ChronosN.exe` とは別の実行ファイルで
起動したい」という要望を検討した。CEFには `CefSettings.browser_subprocess_path`
があり、サブプロセス用の実行ファイルを指定できる。bootstrap構成では、これは
「2つ目のbootstrap(`ChronosSub.exe`)+ 専用DLL(`ChronosSub.dll`)」になる。

実装して動作はした(メイン=`ChronosN.exe`、全サブプロセス=`ChronosSub.exe`、
ページ表示もOK)。ChronosSub.dllは全プロセス種別が exe名から読み込むため、
`OnBeforeChildProcessLaunch` フックに頼らずに済む
(実測でこのフックは renderer と一部の utility にしか掛からず、
GPU/network/storage には掛からないので、`--module` 注入には使えない)。

**しかし、`browser_subprocess_path` を設定すると全サブプロセスに `--no-sandbox`
が付き、サンドボックスが無効化される。** これはCEFの仕様で、
`chrome_main_delegate_cef.cc` に次のようにハードコードされている。
...
つまり **Windowsでは「別のサブプロセスEXE」と「サンドボックス」は排他**であり、
CEF本体にパッチを当てない限り両立しない。本セッションの目的はサンドボックス
有効化なので、サブプロセス分離は採用せず、変更は撤回した
(`browser_subprocess_path` の設定、`ChronosSub.exe`/`ChronosSub.dll` の配置、
診断用の `OnBeforeChildProcessLaunch` フックはいずれもコミットしていない)。

どうしてもサブプロセスを別EXEにしたい場合は、サンドボックスを諦めることになる。
プロセス名で見分けたいだけであれば、サンドボックスを保ったまま実現できる別案として、
サブプロセスの `--type` に応じてプロセスの表示名やクラッシュレポートのタグを
変える、といった方法を検討する余地がある(未調査)。
