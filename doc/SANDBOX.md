...
3. `Sazabi.cpp`
   ...
   * `CSazabi::GetSandboxInfo()` を追加。`CHRONOS_ENABLE_SANDBOX` 未定義の間は
...
* `CHRONOS_ENABLE_SANDBOX` を定義するだけでサンドボックスが有効になる
* 後述チェックリストの回帰確認
...
### 参考: Phase 2 の予備調査(サンドボックス有効での試行)

`CHRONOS_ENABLE_SANDBOX` を定義したビルドでも起動自体はでき、
サブプロセス群も生成された。ただし、

* サンドボックス化されたプロセスはコマンドラインを外部から読めなくなり、
  `--no-sandbox` が消えていることは確認できた
* ウィンドウは表示されるがページが描画されず、30秒程度で全プロセスが終了した
  (イベントログにアプリケーションエラーの記録はなし)
...
実行される**点が挙げられる。ロックダウントークン下では、これらが期待どおりに
動作しない可能性がある。Phase 2 ではまずCEFのログを有効にして
サブプロセスの起動失敗理由を特定するところから始めるとよい。
