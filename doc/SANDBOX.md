...
1. `RunWinMain()` の本体を `try` / `catch(...)` で囲み、捕捉できたら
   独自の例外コードで `RaiseException` する。イベントログの例外コードが
   変われば原因は `RunWinMain()` 内(=`CefExecuteProcess()` の中)、
   変わらなければ静的初期化、と一手で切り分けられる
2. 静的初期化が原因の場合、`theApp` を含むグローバルオブジェクトが
   コンストラクタで何を行っているかを洗い出す
3. storage サービスだけが落ちる理由(ファイルアクセスなのか、
   `--service-sandbox-type=service` 固有の制限なのか)を確認する
