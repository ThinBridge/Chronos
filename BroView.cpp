...
public:
	...
	void Execute() override
	{
		...
		pSnap->strArrayTitles.Copy(visitor->m_strArrayRet);
...
LRESULT CChildView::OnAddressChange(WPARAM wParam, LPARAM lParam)
{
	...
	// アンカー移動などでも履歴は変化するため、アドレス変化を契機にスナップショットを更新する。
	RequestTravelLogRefresh();
	return S_OK;
}
...
LRESULT CChildView::OnCefTravelLogSync(WPARAM wParam, LPARAM /*lParam*/)
{
	...
	if (!pSnap) return 0;
	m_straTravelLog.Copy(pSnap->strArrayTitles);
