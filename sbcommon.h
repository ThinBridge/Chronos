...
#ifdef _CEF87
static TCHAR sgSZB_UA_END[] = _T(") AppleWebKit/537.36 (KHTML, like Gecko;KA-ZUMA) Chrome/87.0.4280.141 Safari/537.36 Chronos/SystemGuard");
#else
static TCHAR sgSZB_UA_END[] = _T(") AppleWebKit/537.36 (KHTML, like Gecko;KA-ZUMA) Chrome/96.0.4664.55 Safari/537.36 Chronos/SystemGuard");
#endif
...
namespace SBUtil
{
	enum ICON_INDEX
	{
		...
		ICON_APP_ABOUT,
		ICON_TOPMOST_WND,
		ICON_TOPMOST_REL_WND,
		ICON_ADD_FAVORITE,
		ICON_ORGANIZE_FAVORITE,
		ICON_GO,
		ICON_PASTE_GO,
		ICON_PASTE_SEARCH,
		ICON_SEARCH,
		...
		for (int i = 0; i < m_ptrArrDlgBrowserID.GetCount(); i++)
		{
			if (m_ptrArrDlgBrowserID.GetAt(i) == nBrowserId)
			{
				...
				if (pDLDlg)
				{
					...
					{
						CString strTitle;
						strTitle.Format(_T("ダウンロード(%d%%) %s"), nProgress, strFileName);
